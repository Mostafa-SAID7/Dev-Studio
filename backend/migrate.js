...
replaceInFile('drizzle.config.ts', [
  ['./server/db/schema.ts', './src/domain/schema.ts']
]);

// Update src/infrastructure/database/index.ts
replaceInFile('src/infrastructure/database/index.ts', [
  ['./schema.js', '../../domain/schema.js']
]);
...
if (fs.existsSync(bPath('src/presentation/routes/api'))) {
  walkSync(bPath('src/presentation/routes/api'), (filepath) => {
    if (filepath.endsWith('.ts')) {
      ...
      content = content.replace(/\.\.\/\.\.\/db\/index\.js/g, '../../../infrastructure/database/index.js');
